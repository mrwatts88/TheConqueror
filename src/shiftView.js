...
export const shiftView = p5 => {
    ...
    if (players['1'].xpos !== undefined) {
        ...
        // player going up
        if ((players['1'].ypos - screensTop) < 2 * BS) {
            let canMove = sC.row;
            let wantToMove = HEIGHT_UNITS - 5;
            let move = Math.min(canMove, wantToMove);
            sC.row -= move;
        ...
        // player going right
        if (p5.width - ((players['1'].xpos - screensLeft) + BS) < 2 * BS + 160) {
        ...
        // player going left
        if ((players['1'].xpos - screensLeft) < 2 * BS) {
            let canMove = sC.col;
            let wantToMove = WIDTH_UNITS - 10;
            let move = Math.min(canMove, wantToMove);
            sC.col -= move;
