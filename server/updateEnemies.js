...
const goUp = (enemy, map, io) => {
    if (processGuy(Math.floor((enemy.ypos - enemy.speed) / BS), undefined, enemy, io)) {
...
const goDown = (enemy, map, io) => {
    if (processGuy(Math.floor((enemy.ypos + enemy.speed + BS - 1) / BS), undefined, enemy, io)) {
        enemy.ypos += enemy.speed;
...
const goLeft = (enemy, map, io) => {
    if (processGuy(undefined, Math.floor((enemy.xpos - enemy.speed) / BS), enemy, io)) {
...
const goRight = (enemy, map, io) => {
    if (processGuy(undefined, Math.floor((enemy.xpos + enemy.speed + BS - 1) / BS), enemy, io)) {
        enemy.xpos += enemy.speed;
...
const goPrevDirection = (enemy, map, io) => {
    switch (enemy.prevDirection) {
        case 'stay': break;
        case 'up': goUp(enemy, map, io); break;
        case 'down': goDown(enemy, map, io); break;
        case 'left': goLeft(enemy, map, io); break;
        case 'right': goRight(enemy, map, io); break;
        default: goRight(enemy, map, io); break;
    }
}
